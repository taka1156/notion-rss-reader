...
import { NotionRepository } from "../infrastructure/notion/notionFeedRepository";
import { RSSFetcher } from "../infrastructure/rss/rssFetcher";
import { RSSParser } from "../infrastructure/rss/rssParser";
...
export class SyncFeedUseCaseImpl implements SyncFeedUseCase {
    ...
    async execute(): Promise<void> {
        ...
        try {
            ...
            info(`Found ${feedConfigs.length} feeds and ${existingUrls.size} existing articles`);
    ...
    private async processFeed(feed: FeedConfig, existingUrls: Set<string>): Promise<void> {
        try {
            info(`Processing ${feed.name} (${feed.url})`);

            const xmlText = await this.rssFetcher.fetchFeed(feed.url);
            const rawItems = this.rssParser.parse(xmlText);

            const entries = rawItems
                .map(item => this.convertToFeedEntry(item, feed.name))
                .filter((entry): entry is FeedEntry => entry !== null && !existingUrls.has(entry.url));
    ...
    private convertToFeedEntry(item: any, sourceName: string): FeedEntry | null {
        ...
        const publishedAt = this.toNotionDate(
            this.stringifyText(item.published || item.pubDate || item["dc:date"]) ||
    ...
    private extractUrl(item: any): string {
        const link = item.link;
        if (typeof link === "string") return link.trim();

        if (Array.isArray(link)) {
            const alt = link.find(l => typeof l === "object" && (l.rel === "alternate" || !l.rel));
            if (alt) {
                return typeof alt === "string" ? alt : (alt.href || alt.url || "").trim();
            }
            return this.stringifyText(link[0]);
        }
    ...
    private stringifyText(value: any): string {
